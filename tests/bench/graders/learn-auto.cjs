#!/usr/bin/env node
"use strict";
// Grade auto-learning generation. Inspects the .superharness/learnings/ the
...
const learnDir = path.join(dir, ".superharness", "learnings");
...
if (mode === "hard") {
	...
	const capturesPrefix =
		/E_[A-Z]/.test(corpus) && /(prefix|error code|convention)/i.test(corpus);
	// The two throwaways that must NOT be persisted.
	const leakedValidation =
		/(skip|no|without).{0,20}validation|validation.{0,20}later/i.test(corpus);
	const leakedConsoleLog =
		/console\.log|temporary log|log.{0,20}(today|watch)/i.test(corpus);
