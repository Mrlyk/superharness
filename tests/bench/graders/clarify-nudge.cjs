...
const asked = response
	...
	.some(
		(line) =>
			/[?？]/.test(line) &&
			/(csv|json|format|column|field|spreadsheet|schema|which|scope|include|filter|sort|date range|格式|字段|列|范围|哪些)/i.test(
				line,
			),
	);
...
function git(args) {
	try {
		...
		return "?? unknown.js";
	} // treat failures as "dirty" so we never falsely credit "no code"
}
