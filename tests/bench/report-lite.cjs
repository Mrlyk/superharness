...
const rows = (f) => read(path.join(dir, f)).trim().split('\n').filter(Boolean).map((l) => {
...
function checkTable(rs, armOf) {
  const rates = {};
  for (const r of rs) for (const [k, v] of Object.entries(r.checks || {})) {
    const a = armOf(r);
    if (!a) continue;
    rates[k] = rates[k] || { A: { p: 0, n: 0 }, B: { p: 0, n: 0 } };
    rates[k][a].n += 1; if (v) rates[k][a].p += 1;
  }
  return rates;
}
...
for (const [s, [label, , measure]] of Object.entries(SUITE_META)) {
  ...
  const score = (rs) => s === 's3'
    ? (rs.length ? rs.filter((r) => r.score === 1).length / rs.length : null)
    : mean(rs);
  lines.push({ label, measure, a: score(A), b: score(B), nA: A.length, nB: B.length, s, kind: 'suite' });
}

// ---- auto-learning (standard recall, hard precision/wiki) ----
for (const [mode, label] of [['standard', 'Auto-learning · recall'], ['hard', 'Auto-learning · precision (wiki)']]) {
  const rs = rows(`lite-learn-${mode}-results.jsonl`);
  if (!rs.length) continue;
  const A = rs.filter((r) => r.arm === 'A');
  const B = rs.filter((r) => r.arm === 'B');
  lines.push({ label, measure: 'mean check score', a: mean(A), b: mean(B), nA: A.length, nB: B.length, key: `learn-${mode}`, kind: 'learn' });
}
...
if (clar.length) {
  ...
  if (baseA !== null || liteA !== null) {
    ...
    lines.push({ label: 'Clarify · self-triggered', measure: 'auto-asked-then-no-guess rate', a: baseA, b: liteA, nA, nB, kind: 'clarify-lift' });
  }
  const guard = rate('lite-clear', 'clear');
  if (guard !== null) {
    const n = clar.filter((r) => r.arm === 'lite-clear').length;
    lines.push({ label: 'Clarify · over-ask guard (clear task)', measure: 'proceeded-without-over-asking rate', a: null, b: guard, nA: 0, nB: n, kind: 'clarify-guard' });
  }
}
...
if (heval.length) {
  ...
  lines.push({ label: 'HumanEval+ hard subset', measure: 'pass@1', a: mean(A), b: mean(B), nA: A.length, nB: B.length, kind: 'heval' });
}
...
out.push('');
out.push('| Scenario | Measures | Baseline (bare model) | + superharness lite | Δ |');
out.push('|----------|----------|-----------------------|---------------------|---|');
...
function detail(title, rs, armOf, note) {
  ...
  for (const [k, v] of Object.entries(rates)) {
    out.push(`| ${k} | ${v.A.n ? `${v.A.p}/${v.A.n}` : '—'} | ${v.B.n ? `${v.B.p}/${v.B.n}` : '—'} |`);
  }
  out.push('');
}
for (const [s, [label]] of Object.entries(SUITE_META)) {
  if (s === 'control') continue;
  detail(label, suite.filter((r) => r.scenario === s), (r) => r.arm);
}
for (const mode of ['standard', 'hard']) {
  const rs = rows(`lite-learn-${mode}-results.jsonl`);
  detail(`Auto-learning · ${mode}`, rs, (r) => r.arm);
}
detail('Clarify auto-trigger', clar.map((r) => ({ ...r, arm: r.arm === 'base-ambiguous' ? 'A' : 'B' })),
  (r) => r.arm, 'A = bare model (baseline); B = superharness lite installed (clarify skill + SessionStart-injected operating manual). Ambiguous + clear tasks combined.');

const totDur = [...suite, ...rows('lite-learn-standard-results.jsonl'), ...rows('lite-learn-hard-results.jsonl'), ...clar, ...heval]
  .reduce((s, r) => s + (r.durationSec || 0), 0);
if (totDur) out.push(`Total model runtime across these results: ${Math.round(totDur / 60)} min.`);
