...
import { copyHookScripts, copySkillDir, listSkillDirs } from "../utils/fs.js";
import { mergeHookConfig } from "../utils/hooks.js";
import { logSuccess } from "../utils/log.js";

export function setupAoneCopilot(
	projectDir: string,
	packageRoot: string,
): void {
			...
			preToolUse: [
				{
					...
					hooks: [
						{
							type: "command",
							command: "node .aone_copilot/hooks/pre-tool-use.js",
							timeout: 30,
						},
					],
				},
			...
			stop: [
				{
					hooks: [
						{
							type: "command",
							command: "node .aone_copilot/hooks/subagent-stop.js",
							timeout: 10,
						},
					],
				},
	...
	logSuccess(
		"Aone Copilot: 已创建 hooks.json (sessionStart + preToolUse + stop)",
	);
}
