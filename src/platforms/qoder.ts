...
function installLite(projectDir: string, packageRoot: string): void {
	...
	// — session-start.cjs guards on session_id to inject only on the first prompt.
		...
		UserPromptSubmit: [
			{
				hooks: [
					{
						type: "command",
						command: "node .qoder/hooks/session-start.cjs",
						timeout: 10,
					},
				],
			},
		...
		Stop: [
			{
				hooks: [
					{
						type: "command",
						command: "node .qoder/hooks/stop-verify.cjs",
						timeout: 15,
					},
					{
						type: "command",
						command: "node .qoder/hooks/stop-learn.cjs",
						timeout: 15,
					},
				],
			},
