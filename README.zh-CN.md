...
**Lite（默认）** —— 存量项目维护，只装 `discover` / `clarify` / `test` / `learn`：

```bash
superharness init
```

装好后正常写代码即可：让 AI 跑一次 `discover` 扫代码库生成规范；之后 clarify / test / learn 由 hook 自动触发，无需任何命令。
...
**Less is more。** lite 只装四个会复利的能力——记忆（`learn`）、规范（`discover`）、澄清（`clarify`）、收尾测试（`test`），其余一概不装。
...
├── learnings/                        # 自学习沉淀
│   └── INDEX.md
└── spec/                             # 项目规范（discover 生成，hook 注入入口）
    └── guides/index.md
```
...
lite 默认只安装 `discover` / `clarify` / `test` / `learn` 这一组；full 安装下表全部。

| 分类 | Skill | 用途 |
|------|-------|------|
| Lite（默认）| `discover` | 扫描代码库生成精简项目规范：`AGENTS.md`/`CLAUDE.md` 薄入口 + `.superharness/spec/` |
| | `clarify` | 需求模糊时自动追问、先问再写；清晰时不挡路 |
| | `test` | 「收工前先跑一遍」终检门控 + Spec/Code 双规范审查 |
| | `learn` | 会话经验沉淀到 `.superharness/learnings/`，自动注入后续会话 |
