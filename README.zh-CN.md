...
Superharness 有**两种模式**，在 `init` 时选定。维护存量项目？用 **lite**（默认）。从零开始做新项目、想要完整纪律？用 **full**（`--full`）。两者共用同一套 skill 和 hook，full 只是在上面叠加了重量级的 task / worktree / 审查工作流。

### Lite（默认）—— 存量项目维护

随着模型能力增强，原来那套从头到尾的完整工作流对**存量项目维护**偏重。`superharness init` 默认安装 **lite** 精简版——只保留随模型增强仍持续增值的四个能力：
...
- **clarify** —— 主动的需求澄清，模型不缺定时先问再写
- **test** —— 「收工前先跑一遍」的终检门控：逼模型在声明完成前用文档示例 + 对抗性边界用例跑过自己的代码；另有显式的 Spec + Code Review 双规范检查技能，整个任务完成后跑一次。

没有强制工作流，没有 `tasks/` / `worktree` 脚手架。自学习和终检门控经钩子自动运行——直接写代码，框架不挡路。`superharness init --full` 安装下面的完整 greenfield 工作流。

### 基准测试

同任务、同模型（Sonnet）、真实端到端 `claude -p` 运行、确定性程序评分的 A/B 对照。A 臂是裸模型，B 臂是同一模型在 fixture 里跑过 `superharness init`（lite），唯一变量就是 lite 的四大能力。单元格为平均检查得分（代码行为 pass@1）。完整方法学与分项检查表见 [docs/benchmark-lite.md](docs/benchmark-lite.md)。
...
| 需求澄清 | 0% | 33% | **+33pp** |
| 澄清·自触发 | 0% | 67% | **+67pp** |
| 澄清·不过度追问（清晰任务） | — | 100% | 守住 |
| 收尾测试 | 40% | 53% | **+13pp** |
| 规范遵循 | 100% | 100% | 持平 |
| HumanEval+ 困难子集 | 30% | 57% | **+27pp** |
| 控制组·HumanEval/0–9 | 100% | 100% | 无回归 |

提升最大的是会复利的能力：跨会话记忆（+80pp）、自动学习沉淀正确的持久规则同时拒绝一次性指令（+71pp）、以及 clarify skill 在模糊请求上自动触发、清晰请求上不过度追问。在社区 **HumanEval+ 困难子集**（Sonnet 基线失败的 7 题，每臂 8 次）上，verify-before-done 终检门控逼模型在收工前用边界用例跑过自己的解，把 pass@1 从 30% 拉到 57%。控制组（简单的 HumanEval/0–9）确认无回归。复现：`tests/bench/lite-suite.sh`、`lite-learn.sh [--hard]`、`lite-clarify.sh`、`heval-lite.sh --plus`。


### Full（`--full`）—— 新项目（greenfield）
...
**小改动不需要走 full 工作流。** Session-start hook 在每次 AI 会话启动时自动将项目规范和调度协议注入到上下文，AI 工具在任何对话中都会遵循这些规范。修一个 bug、调一个配置、改几行代码——直接说，不需要输入任何 superharness 命令。
