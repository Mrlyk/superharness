...
- **sh-test** —— 整个任务完成后跑一次的终检：Spec + Code Review 双规范审查 + 测试套件。某次改动是否要补测试由模型按改动复杂度自行判断，不是强制门控；判断需要时写正式的项目单测（提交进仓库、长期沉淀），而非跑完即删的一次性脚本。
- **sh-learn** —— 会话中的纠正、踩坑、决策自动沉淀到 `.superharness/learnings/`，并注入后续会话。

每个能力都是一个 skill：session-start hook 在会话开头注入项目规范 + 过往沉淀，stop hook 在回合结束跑后台学习。
...
| | `sh-test` | 任务完成后终检：Spec/Code 双规范审查 + 测试套件（是否补测试由模型按复杂度自行判断） |
